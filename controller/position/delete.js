...
//send delete request to alpaka
//send data to client
exports.closeAllPosition = async (req, res, next) => {
	...
	try {
		...
		//deleting positions from alpaka
		let positions = null;
		...
		try {
			...
			logger.error(`Alpaca delete positions item error, ${error.response}`);
			return next(
				new ErrorResponse(
					`Alpaka Delete positionsError : ${JSON.stringify(
						error.response
					)}`,
					500
				)
		...
		if (!positions || positions === null) {
			return next(
				new ErrorResponse("Alpaka delete positions item cannot be null", 400)
			);
		}
		...
		logger.error(`Caught delete all positions from user error : ${JSON.stringify(error)}`);
		next(error);
	}
};
...
exports.closeSinglePosition = async (req, res, next) => {
	...
	try {
		...
		logger.error(`Caught delete single position : ${JSON.stringify(error)}`);
		next(error);
	}
}